    ...
    _next_spectator_frame(0)
{
    ...
    */
   _udp.Init(localport, &_poll, this, hPlatform, localProductUserId);
...
void
Peer2PeerBackend::AddRemotePlayer(EOS_ProductUserId peer,
                                  int queue)
{
   /*
   ...
   _endpoints[queue].Init(&_udp, _poll, queue, peer, _local_connect_status);
   _endpoints[queue].SetDisconnectTimeout(_disconnect_timeout);
   _endpoints[queue].SetDisconnectNotifyStart(_disconnect_notify_start);
   _endpoints[queue].Synchronize();
}

GGPOErrorCode Peer2PeerBackend::AddSpectator(EOS_ProductUserId peer)
{
   ...
   _spectators[queue].Init(&_udp, _poll, queue + 1000, peer, _local_connect_status);
                            ...
                            GGPOPlayerHandle *handle)
{
   if (player->type == GGPO_PLAYERTYPE_SPECTATOR) {
      return AddSpectator(player->u.remote._EOS_ProductUserId);
   }
   ...
   *handle = QueueToPlayerHandle(queue);
   ...
   if (player->type == GGPO_PLAYERTYPE_REMOTE) {
      AddRemotePlayer(player->u.remote._EOS_ProductUserId, queue);
   }
   return GGPO_OK;
}
...
void
Peer2PeerBackend::OnMsg(EOS_ProductUserId &from, UdpMsg *msg, int len)
{
   for (int i = 0; i < _num_players; i++) {
