 ...
 */
void
VectorWar_Init(HWND hwnd, unsigned short localport, int num_players, GGPOPlayer *players, int num_spectators, EOS_ProductUserId localProductUserId, EOS_HPlatform hPlatform)
{
...
#else
   result = ggpo_start_session(&ggpo, &cb, "vectorwar", num_players, sizeof(int), localport, localProductUserId, hPlatform);
#endif
 ...
 */
void
VectorWar_InitSpectator(HWND hwnd, unsigned short localport, int num_players, EOS_ProductUserId host_user)
{
   GGPOErrorCode result;
   renderer = new GDIRenderer(hwnd);
   ...
   localport = 0;
   result = ggpo_start_spectating(&ggpo, &cb, "vectorwar", num_players, sizeof(int), localport, host_user);
